...
// 1440 and 390, in the dark and the light theme (THEMES=dark for one; violations
// only). Needs axe-core in node_modules (npm i axe-core).
...
for (const pg of pages) for (const w of [1440, 390]) for (const theme of themes) {
  ...
  bad += v.length;
  console.log(`${v.length ? 'FAIL' : 'PASS'}  ${pg} ${w} ${theme}${v.length ? '\n    ' + v.join('\n    ') : ''}`);
  await ctx.close();
}
await b.close();
